/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { OrganizationCreateNestedManyWithoutDiamondPricesInput } from "./OrganizationCreateNestedManyWithoutDiamondPricesInput";
import { ValidateNested, IsOptional, IsInt } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class DiamondPriceCreateInput {
  @ApiProperty({
    required: false,
    type: () => OrganizationCreateNestedManyWithoutDiamondPricesInput,
  })
  @ValidateNested()
  @Type(() => OrganizationCreateNestedManyWithoutDiamondPricesInput)
  @IsOptional()
  @Field(() => OrganizationCreateNestedManyWithoutDiamondPricesInput, {
    nullable: true,
  })
  organizations?: OrganizationCreateNestedManyWithoutDiamondPricesInput;

  @ApiProperty({
    required: true,
    type: Number,
  })
  @IsInt()
  @Field(() => Number)
  value!: number;
}

export { DiamondPriceCreateInput as DiamondPriceCreateInput };
