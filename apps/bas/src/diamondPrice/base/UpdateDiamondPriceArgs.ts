/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DiamondPriceWhereUniqueInput } from "./DiamondPriceWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { DiamondPriceUpdateInput } from "./DiamondPriceUpdateInput";

@ArgsType()
class UpdateDiamondPriceArgs {
  @ApiProperty({
    required: true,
    type: () => DiamondPriceWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => DiamondPriceWhereUniqueInput)
  @Field(() => DiamondPriceWhereUniqueInput, { nullable: false })
  where!: DiamondPriceWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => DiamondPriceUpdateInput,
  })
  @ValidateNested()
  @Type(() => DiamondPriceUpdateInput)
  @Field(() => DiamondPriceUpdateInput, { nullable: false })
  data!: DiamondPriceUpdateInput;
}

export { UpdateDiamondPriceArgs as UpdateDiamondPriceArgs };
